...
def input_compute(x,k=16,metric="euclidean",knn_matrix=None): #x représente un dataset 
  # if knn_matrix is None:
    ...
    # build_annoy_index(x,"index/ind_"+dt,metric=metric,build_index_on_disk=True)
...
def pn_loss_g(y_pred,distance=euclidean_distance,margin=1.):    
  ...
  # tf.print("min :", minimum_distance)
  # tf.print("pnk_before :", K.mean(K.maximum(anchor_positive_distance - minimum_distance + margin, 0)))
